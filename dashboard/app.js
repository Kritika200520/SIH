...
/**
 * Minimal Layout Tab Switcher
 */
function switchPanelTab(panel, tabId) {
  if (panel === 'left') {
    document.querySelectorAll('.tab-btn-left').forEach(b => b.classList.remove('tab-btn--active'));
    const activeBtn = document.getElementById(`tab-btn-${tabId}`);
    if (activeBtn) activeBtn.classList.add('tab-btn--active');
    
    const navEl = document.getElementById('tab-content-nav');
    const visEl = document.getElementById('tab-content-vision');
    if (navEl) navEl.style.display = (tabId === 'nav') ? 'block' : 'none';
    if (visEl) visEl.style.display = (tabId === 'vision') ? 'block' : 'none';
  } else if (panel === 'right') {
    document.querySelectorAll('.tab-btn-right').forEach(b => b.classList.remove('tab-btn--active'));
    const activeBtn = document.getElementById(`tab-btn-${tabId}`);
    if (activeBtn) activeBtn.classList.add('tab-btn--active');
    
    const bcEl = document.getElementById('tab-content-broadcast');
    const thEl = document.getElementById('tab-content-threat');
    if (bcEl) bcEl.style.display = (tabId === 'broadcast') ? 'block' : 'none';
    if (thEl) thEl.style.display = (tabId === 'threat') ? 'block' : 'none';
  }
}

/**
 * Minimal Layout Panel Minimize / Expand Toggle
 */
function togglePanel(side) {
  ...
  if (panel) {
    ...
    if (btn) {
      btn.innerHTML = panel.classList.contains('panel--collapsed') ? '+' : '&minus;';
    }
  }
}
